...
public class BankerClass {
    ...
    public void setAlloction() {  //设置已分配资源矩阵Alloction
    ...
    public void printSystemVariable(){      //显示此时资源分配的情况
    ...
    public void BankerAlgorithm() {     //银行家算法
        ...
        if (Request[num][0] <= Need[num][0] && Request[num][1] <= Need[num][1] && Request[num][2] <= Need[num][2]) {     //判断Request是否小于Need
            if (Request[num][0] <= Available[0] && Request[num][1] <= Available[1] && Request[num][2] <= Available[2]) {   //判断Request是否小于Alloction
                ...
                System.out.println("当前没有足够的资源可分配，进程P" + num + "需等待。");      //当前没有足够的资源可分配
                T = false;
            }
        } else {
            System.out.println("进程P" + num + "请求已经超出最大需求量Need.");  //请求经超出最大需求量
            T = false;
        }

        if(T == true){
            printSystemVariable();  //走到此时，请求允许分配资源，调用显示情况，进入安全算法
